...
With `selection="ranked"` step 3 is replaced by ikNN-style ranking: the `n_subspaces` best candidates by individual score, weighted by that score or uniformly. Selection needs no random seed and gives the same model on every platform and for any order of the training rows: candidates are enumerated in a fixed order, leave-one-out needs no random split, points tied at the distance of the $k$-th neighbour share its votes instead of being picked by the neighbour search, and losses closer than $10^{-12}$ count as ties, which go to the candidate enumerated first. CI checks this against committed results on Linux, macOS and Windows.
