...
Three details make the result identical on every platform and for any order of the training rows:
...
- **Complete tie groups.** The search is asked for one candidate more than $k$, and asked again for twice as many for any sample whose farthest candidate is not clearly beyond $d^\ast$, until every point tied at $d^\ast$ is in the list.
- **Order-free totals.** Under uniform weights a class's total is an exact count of its closer points plus the share times its count of tied points. Under distance weights the candidates are put in order of distance and then index and summed one after another, so tied points contribute identical weights at the same positions whatever their indices.

The same model gives the leave-one-out votes, the k-fold votes and the predictions, so selection and prediction treat ties alike. Compared with scikit-learn's own leave-one-out query, the rule costs about a quarter more time for the leave-one-out step, measured at 2,000 to 50,000 samples; on data without ties the votes are the same.
